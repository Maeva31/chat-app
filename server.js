...
case '/removeadmin':
 ...
 if (cmd === '/removemodo') {
  ...
  if (users[targetName]) {
    ...
    // ⛔ Si l'utilisateur était invisible (ancien admin), désactive son invisibilité
  ...
  if (users[targetName]) {
    ...
    // ⛔ Si l'utilisateur était invisible (admin), désactive son invisibilité
    if (users[targetName].invisible) {
      users[targetName].invisible = false;
      const targetSocketId = users[targetName].id;
      const targetChannel = userChannels[targetSocketId];
      ...
      io.to(targetSocketId).emit('server message', "Vous avez perdu votre rôle d'administrateur, le mode invisible est désactivé.");
      io.to(targetChannel).emit('chat message', {
        username: 'Système',
        message: `${targetName} est maintenant visible.`,
        timestamp: new Date().toISOString(),
